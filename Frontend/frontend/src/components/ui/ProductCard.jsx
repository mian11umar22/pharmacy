...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-card p-3 md:p-4 group relative flex flex-col h-full animate-fade-up hover:shadow-hover hover:-translate-y-1 transition-all duration-300">
            ...
            {/* Image */}
            <Link to={`/product/${product.id}`} className="block relative overflow-hidden rounded-xl bg-gray-50 aspect-square mb-3 md:mb-4">
                <img
                    src={product.image}
                    alt={product.name}
            ...
            <div className="flex flex-col flex-grow">
                ...
                <Link to={`/product/${product.id}`} className="font-semibold text-secondary text-sm md:text-base mb-2 md:mb-3 hover:text-primary transition-colors line-clamp-2 leading-snug">
                    {product.name}
                </Link>
